...
class repos {
    ...
    has_been_stored() {
        if (this.ss.hasOwnProperty(`${uid_to_json_key(this.uname)}_repos`)) {
            return true;
        }
        return false;
    }
    ...
    check_validity() {
        ...
        if (
            new Date().getTime() <
            new Date(JSON.parse(this.ss.getItem(`${uid_to_json_key(this.uname)}_repos`)).timestamp + this.time_interval_minutes * 60000)
        ) {
            return false;
        }
        return true;
        ...
        if (this.check_validity()) {
            fetch(this.url)
                ...
                .then((output) => {
                    ...
                    this.ss.setItem(
                        `${uid_to_json_key(this.uname)}_repos`,
                        JSON.stringify({
                            user: this.uname,
...
let obj_repos = new repos(uname);

obj_repos.main();

if (!sessionStorage.hasOwnProperty(`${uid_to_json_key(uname)}_repos`)) {
    setTimeout(() => {
        main_repos();
    }, 1600);
} else {
...
function main_repos() {
    ...
    if (ss_repos.data.length == 0) {
        ...
        nothing_msg.innerHTML = `There are no repositories for <span>${uname}</span>.`;
        return;
    }
